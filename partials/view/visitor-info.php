...
if ( $num_ips > 0 ) {
	if ( isset( $_GET['ip'] ) && $_GET['ip'] != '' ) {
		...
		$visitor_ip = WHTP_Geolocation::get_ip_address();
	}
	...
	*/
	$country_name = WHTP_Geolocation::get_country_name_by_ip( $visitor_ip );


	$agent_ids = WHTP_Ip_Hits::agent_ids_from_ip_id( $ip_id );

	// Get the browsers used by the user.
...
<div class="mdl-layout whtps-content mdl-grid">
	...
	<?php esc_attr_e( 'Who Hit The Page Hit Counter', 'whtp' ); ?>
	</h2>
	<p></p>
	</div>
	<div class="mdl-row mdl-grid mdl-grid mdl-cell mdl-cell--12-col">
	<div class="whtp-updates mdl-card mdl-shadow--2dp mdl-cell mdl-cell--12-col">
		<div class="mdl-card__supporting-text mdl-color-text--grey-600">
			<p><?php esc_attr_e( 'To start viewing the details of a single visitor, select the visitor\'s IP below and click "View Details", do the same to view another IP. Please select an IP to see more details about that IP Address.', 'whtp' ); ?></p>
	<?php
	...
	if ( $ip_results ) :
	   ...
	   <form name="select_ip" method="post" action="" >
		...
		<input style="display:inline;" type="submit" value="<?php esc_attr_e( 'View Details', 'whtp' ); ?>" class="button button-primary mdl-button mdl-js-button mdl-js-ripple-effect" />
	   </form>
		<?php
	endif;
	?>
	...
	<div class="mdl-grid mdl-grid mdl-cell mdl-cell--12-col">
		<h2 class="mdl-card__title-text">
	<?php esc_attr_e( 'View Visitor\'s Behaviour (IP:', 'whtp' ); ?> <?php echo $visitor_ip; ?>)
		</h2> 
	</div>
	...
	<div class="mdl-row whtp-updates mdl-cell mdl-cell--12-col">
		...
		<?php esc_attr_e( 'Visitor Statistics!', 'whtp' ); ?>
					</div>
					<div class="mdl-card__supporting-text mdl-color-text--grey-600">
		...
		<?php echo sprintf( __( 'Location: %s', 'whtp' ), $country_name ); ?>
					...
					<div class="mdl-card__supporting-text mdl-color-text--grey-600">
		<?php esc_attr_e( 'First Visit :', 'whtp' ); ?> <?php echo $info_result->datetime_first_visit; ?>
					</div>
					<div class="mdl-card__supporting-text mdl-color-text--grey-600">
		<?php esc_attr_e( 'Last Visit : ', 'whtp' ); ?><?php echo $info_result->datetime_last_visit; ?>
					</div>            
				</div>  
			</div>
			...
			<div class="mdl-color--white mdl-cell mdl-cell--6-col">
				<div class="whtp-updates mdl-card mdl-shadow--2dp mdl-cell mdl-cell--12-col">
					<div class="mdl-card__title mdl-color--teal-300">
		<?php esc_attr_e( 'Browsers & Pages!', 'whtp' ); ?>
					</div>            
					<div class="mdl-card__supporting-text mdl-color-text--grey-600">
						<strong><?php esc_attr_e( 'Pages Visited by this user', 'whtp' ); ?></strong>
					</div>
		<?php
		if ( ! $pages_visited || count( $pages_visited ) <= 0 ) :
			?>
		  <div class="mdl-card__supporting-text mdl-color-text--grey-600">
								<?php esc_attr_e( 'No Pages Visited', 'whtp' ); ?>
		  </div>
			<?php
						else :
						...
						?>
					<div class="mdl-card__supporting-text mdl-color-text--grey-600">
					<strong><?php esc_attr_e( 'The User Has The following Browsers', 'whtp' ); ?></strong>
					</div>
		<?php
		if ( ! $browsers || 0 == count( $browsers ) ) :
			?>
		  <div class="mdl-card__supporting-text mdl-color-text--grey-600">
								<?php esc_attr_e( 'No Browser(s)', 'whtp' ); ?>
		  </div>
			<?php
						else :
	...
	<div class="mdl-row mdl-grid mdl-grid mdl-cell mdl-cell--12-col">
		<div class="whtp-updates mdl-card mdl-shadow--2dp mdl-cell mdl-cell--12-col">
			...
			<div class="mdl-card__supporting-text mdl-color-text--grey-600">
				<?php include_once WHTP_PLUGIN_DIR_PATH . 'partials/disclaimer.php'; ?>
			</div>
	...
	*/
	...
	if ( is_admin() ) {
	 ...
	 <div class="whtp-updates mdl-card mdl-shadow--2dp mdl-cell mdl-cell--12-col">
	  <div class="mdl-card__title mdl-color--teal-300">
				<h2 class="mdl-card__title-text">
				<?php esc_attr_e( 'Sorry no results found', 'whtp' ); ?>
				</h2>
	  </div>
	  <div class="mdl-card__supporting-text mdl-color-text--grey-600">
				<p class="about-description">
		<?php esc_attr_e( 'It seems there are no IP Addresses registered in your database at the moment. Maybe I\'ve done something wrong or you have missed some steps during setup.', 'whtp' ); ?>
				</p>
				<br />
				<p class="about-description">
		<?php esc_attr_e( 'Please check if you have done the following', 'whtp' ); ?>
				</p>
				<br />
				<ul>
					<li>
						<span class="welcome-icon welcome-learn-more">
		<?php wp_kses_post( 'Included the shortcode <code>[whohit]Page Name[/whohit]</code> on your pages. If not, click get started below.', 'whtp' ); ?> 
						</span>
					</li>
					<li>
						<span class="welcome-icon welcome-learn-more">
		<?php esc_attr_e( 'Imported all Geo Location data via the <a href="admin.php?page=whtp-import-export">Export/ Import</a> page.', 'whtp' ); ?>
						</span>
					</li>
					<li>
						<span class="welcome-icon welcome-learn-more">
		<?php esc_attr_e( 'If you haven\'t done this, click "Get Started" below.', 'whtp' ); ?>
						</span>
					</li>
				</ul>
				<a href="admin.php?page=whtp-help" class="button button-primary button-hero">
		<?php esc_attr_e( 'Get Started', 'whtp' ); ?>
				</a>
	  </div>
	 </div>  
	</div>
